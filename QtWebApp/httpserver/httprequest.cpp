...
HttpRequest::HttpRequest(const QSettings *settings) {
  ...
  currentSize = 0;
  expectedBodySize = 0;
  maxSize = settings->value("maxRequestSize", "16000").toLongLong();
  maxMultiPartSize =
      settings->value("maxMultiPartSize", "1000000").toLongLong();
  tempFile = nullptr;
}
...
#endif
  qint64 toRead = maxSize - currentSize +
...
#endif
    qint64 toRead = expectedBodySize - bodyData.size();
      ...
      if (line.startsWith("--" + boundary)) {
        ...
        if (fileName.isEmpty() && !fieldName.isEmpty()) {
          ...
          if (uploadedFile) {
            ...
            qDebug("HttpRequest: uploaded file size is %lli",
                   uploadedFile->size());
          } else {
